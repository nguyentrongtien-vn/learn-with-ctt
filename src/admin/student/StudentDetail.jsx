...
function ActivityLine({ values }) {
  ...
  const yTicks = [yMax, yMax * 0.75, yMax * 0.5, yMax * 0.25, 0].map((tick) => Math.round(tick));

  return (
    <svg className="su-activity-chart" width="100%" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" role="img" aria-label="Biểu đồ hoạt động học tập theo thời gian">
      <defs>
      ...
      <text className="su-chart-axis-title su-chart-axis-title-y" x="13" y="100" transform="rotate(-90 13 100)">Số hoạt động</text>
      <text className="su-chart-axis-title" x="326" y="213" textAnchor="middle">Thời gian</text>
      {yTicks.map((tick, index) => {
        const y = chartTop + (index / 4) * (chartBottom - chartTop);
        return <g key={tick}><line className="su-chart-grid" x1={chartLeft} x2={chartRight} y1={y} y2={y} /><text className="su-chart-label" x="43" y={y + 3} textAnchor="end">{tick}</text></g>;
      })}
      <line className="su-chart-axis" x1={chartLeft} x2={chartLeft} y1={chartTop} y2={chartBottom} />
      <line className="su-chart-axis" x1={chartLeft} x2={chartRight} y1={chartBottom} y2={chartBottom} />
      <path d={area} fill="url(#su-area-fill)" />
      <path d={path} fill="none" stroke="var(--su-primary)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      {points.map((point, index) => (
        <g key={index}><circle cx={point[0]} cy={point[1]} r="3.5" fill="#fff" stroke="var(--su-primary)" strokeWidth="2" /><text className="su-chart-value" x={point[0]} y={point[1] - 9} textAnchor="middle">{values[index]}</text><text className="su-chart-label" x={point[0]} y="190" textAnchor="middle">{xLabels[index]}</text></g>
      ))}
    </svg>
  );
}
      ...
      <div className="su-page">
        ...
        <div className="su-detail-head">
          <div className="su-detail-who">
            <div className="su-avatar su-avatar-lg" style={{ background: student.color }}>
              {student.initials}
            </div>
            <div>
              <div className="su-detail-name">
                {student.name}
                <span className="su-badge su-badge--dot su-badge--active">Hoạt động</span>
              </div>
              <div className="su-detail-meta">
                MSSV: {student.mssv} &nbsp;·&nbsp; Email: {student.email} &nbsp;·&nbsp; Tham gia: {detail.joinedAt}
              </div>
            </div>
          </div>
          <button className="su-btn-primary" onClick={() => window.alert("Tính năng nhắn tin sẽ được bổ sung trong phiên bản tiếp theo.")}>Nhắn tin</button>
        </div>
            ...
            <div className="su-list">
              {detail.questionBank.map((question) => (
                <div className="su-list-row" key={question.subject}>
                  <div className="su-list-left">
                    <div className="su-list-icon" style={{ background: "var(--su-purple-soft)", color: "var(--su-purple)" }}>
                      <IconQuestion />
                    </div>
            ...
            <div className="su-list">
              {detail.recentQuizzes.map((quiz, index) => (
                <div className="su-quiz-row" key={index}>
                  ...
                  <div className={`su-quiz-score su-quiz-score--${quiz.status}`}>{quiz.score}</div>
                </div>
              ))}
            </div>
          </div>
