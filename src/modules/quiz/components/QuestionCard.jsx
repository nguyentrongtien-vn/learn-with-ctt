...
export default function QuestionCard() {
  const { quizQuestions, activeQuestionIndex, handleSelectQuizAnswer } = useApp();
  const q = quizQuestions[activeQuestionIndex];

  if (!q) return null;

  return (
    <div className="flex flex-col gap-4 min-h-0 flex-1 justify-between select-none">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 shrink-0">
        {q.options.map((opt) => {
          ...
          const isCorrect = q.isAnswered && opt.key === q.correctAnswer;
          const isWrong = q.isAnswered && isSelected && !q.isCorrect;

          let borderStrip = null;
          let badge = null;
          let letterBg = 'bg-surface-container text-on-surface-variant';
          let ringStyle = 'border-surface-container-high/50 hover:bg-surface-container-low/70 hover:border-primary/40';

          if (q.isAnswered) {
            if (isCorrect) {
              borderStrip = <div className="absolute left-0 top-0 bottom-0 w-2 bg-secondary rounded-l-2xl" />;
              letterBg = 'bg-secondary text-white';
              ringStyle = 'border-secondary/50 bg-secondary-container/20 ring-2 ring-secondary/30';
              badge = (
              ...
              borderStrip = <div className="absolute left-0 top-0 bottom-0 w-2 bg-error rounded-l-2xl" />;
              letterBg = 'bg-error text-white';
              ringStyle = 'border-error/50 bg-error-container/20 ring-2 ring-error/30';
              badge = (
          ...
          return (
            <button
              key={opt.key}
              type="button"
              onClick={() => handleSelectQuizAnswer(activeQuestionIndex, opt.key)}
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2 pl-2">
                <div className="flex items-center gap-3">
                  <span className={`w-8 h-8 rounded-xl text-[14px] flex items-center justify-center font-extrabold shadow-2xs shrink-0 ${letterBg}`}>
                    {opt.key}
                  </span>
          ...
          <div className="flex items-center justify-between pb-2.5 border-b border-surface-container-high/40">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[14px] font-bold text-on-surface">
                {q.isCorrect ? 'Chính xác · Phân tích khái niệm' : 'Chưa chính xác · Phân tích nguyên nhân'}
              </span>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center text-left">
            ...
            <div className="md:col-span-4 bg-surface-container-low/80 rounded-xl p-3.5 flex flex-col gap-2 border border-surface-container-high/30">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-[13px] font-extrabold ${q.isCorrect ? 'text-secondary' : 'text-primary'}`}>
                  {q.isCorrect ? '+5%' : '+3%'}
                </span>
              </div>
